...
- fix: keep backward compatibility with migration files generated by `aerich<=0.9.1` (no `MODELS_STATE` section) — `aerich` commands now print a warning instead of raising an error, recommending the user run `aerich fix-migrations`. Set the env var `AERICH_NO_OLD_FORMAT_WARNING=1` or `no_old_format_warning = true` under `[tool.aerich]` in `pyproject.toml` to silence the warning ([#516])
...
[#516]: https://github.com/tortoise/aerich/issues/516
